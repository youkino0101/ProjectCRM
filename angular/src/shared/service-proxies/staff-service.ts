...
export class StaffServiceProxy {
    ...
    getSession(): Observable<StaffDto> {
        let url_ = this.baseUrl + "/api/services/app/Staff/GetStaffSession";
        url_ = url_.replace(/[?&]$/, "");
    ...
    protected processGetSession(response: HttpResponseBase): Observable<StaffDto> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
